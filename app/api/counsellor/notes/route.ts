...
export async function GET(request: Request) {
  try {
    ...
    const notes = (data || []).map(row => ({ id: row.id, student_id: row.student_id, author_id: row.author_id, created_at: row.created_at, text: decryptText(row.ciphertext, row.iv, row.auth_tag) }))
