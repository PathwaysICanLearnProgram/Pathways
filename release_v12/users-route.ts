...
export async function POST(request: Request) {
  try {
    ...
    const participantType = body.participantType === 'client' ? 'client' : 'student'
    ...
    if (role === 'student') {
      const { error: detailsError } = await supabase.from('student_details').upsert({ user_id: id, stage, career_focus: participantType === 'client' ? 'Personal career development' : 'Exploring', participant_type: participantType })
      if (detailsError) throw detailsError
    }
    await supabase.from('audit_events').insert({ actor_id: user.id, action: 'account_created', entity_type: 'profile', entity_id: id, metadata: { role, participant_type: role === 'student' ? participantType : null, stage: role === 'student' ? stage : null } })
